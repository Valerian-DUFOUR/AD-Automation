<#
.SYNOPSIS
    Rapport Active Directory TOUT-EN-UN : un seul classeur Excel (.xlsx) avec un
    tableau de bord KPI exécutif et une feuille de détail par thème.

.DESCRIPTION
    Script auto-suffisant qui collecte l'ensemble des informations AD en une
    passe et produit un « giga rapport » unique :

        1. « Tableau de bord (KPI) » : synthèse exécutive (comptes, ordinateurs,
           sécurité, hygiène) avec niveaux de risque colorés et graphiques.
        2. Feuilles de détail : Utilisateurs, Ordinateurs, Comptes inactifs,
           Ordinateurs inactifs, Comptes privilégiés, Kerberoastables,
           Mots de passe à risque, Groupes, Trusts, GPO, cpassword (GPP),
           LAPS, Sans BitLocker, Doublons, Échéances, Santé DC, AD CS.

    Les modules/optionnels absents (GroupPolicy, ADCS…) sont ignorés sans erreur.
    Toutes les dates sont au format JJ-MM-AAAA. Lecture seule.

    NB : pour l'analyse la plus fine d'un domaine précis, utilisez le script
    dédié correspondant (délégations ACL complètes, scan SNMP, protocoles, audit
    SMB/NTFS, capture live LDAP… ne sont pas inclus ici).
...
.NOTES
    ...
    (auto-installé). À lancer sur un DC avec des droits d'administration pour un
    rapport complet. Auteur : Valérian DUFOUR / Claude
#>
[CmdletBinding()]
param(
    [string]$SearchBase,
...
$cProps = @('Name','Description','OperatingSystem','OperatingSystemVersion','whenCreated','LastLogonDate','DistinguishedName','Enabled')
$computers = Get-ADComputer -Filter * -Properties $cProps @baseParam

Write-Step "Chargement des groupes..."
$groups = Get-ADGroup -Filter * -Properties GroupCategory,GroupScope,member,whenCreated,Description @baseParam

# ---- Utilisateurs (détail) ----
$usersReport = $users | ForEach-Object {
    [PSCustomObject]@{
        'Nom du compte'=$_.SamAccountName; 'Email'=$_.mail; 'Prénom'=$_.GivenName; 'Nom'=$_.Surname; 'Nom complet'=$_.DisplayName
...
$compReport = $computers | ForEach-Object {
    $d = Days-Since $_.LastLogonDate
    [PSCustomObject]@{
        'Nom du PC'=$_.Name; 'Description'=$_.Description; 'OS'=$_.OperatingSystem; 'Version OS'=$_.OperatingSystemVersion
        'Date de création'=$_.whenCreated; 'Dernière connexion'=$_.LastLogonDate; 'Statut'=(if($_.Enabled){'Activé'}else{'Désactivé'})
        'Jours inactif'=$d; 'Emplacement (OU)'=(Get-OUFromDN $_.DistinguishedName); 'Obsolète'=(Get-ObsolescenceStatus $_.OperatingSystem)
    }
} | Sort-Object 'Nom du PC'

# ---- Inactifs ----
$userInactive = $usersReport | Where-Object { $_.Statut -eq 'Activé' } | ForEach-Object {
    $ref = if ($_.'Dernière connexion') { $_.'Dernière connexion' } else { $_.'Date de création' }
    $d = Days-Since $ref
    if ($d -ge $InactiveDays) { $_ | Add-Member -NotePropertyName 'Jours inactif' -NotePropertyValue $d -PassThru }
} | Sort-Object 'Jours inactif' -Descending
$compInactive = $compReport | Where-Object { $_.Statut -eq 'Activé' -and $_.'Jours inactif' -ne $null -and $_.'Jours inactif' -ge $InactiveDays } | Sort-Object 'Jours inactif' -Descending

# ---- Comptes privilégiés ----
Write-Step "Analyse des comptes privilégiés..."
$privRows = New-Object System.Collections.Generic.List[object]
$privDN = New-Object System.Collections.Generic.HashSet[string]
$privCount = @{}
foreach ($pg in @(@{N='Domain Admins';S="$($domain.DomainSID.Value)-512"},@{N='Enterprise Admins';S="$($domain.DomainSID.Value)-519"},@{N='Schema Admins';S="$($domain.DomainSID.Value)-518"},@{N='Administrators';S='S-1-5-32-544'})) {
...
$kerb = $users | Where-Object { $_.SamAccountName -ne 'krbtgt' -and @($_.servicePrincipalName).Count -gt 0 } | ForEach-Object {
    [PSCustomObject]@{ 'Nom du compte'=$_.SamAccountName; 'Activé'=(if($_.Enabled){'Activé'}else{'Désactivé'}); "MDP n'expire jamais"=(if($_.PasswordNeverExpires){'Oui'}else{'Non'}); 'SPN'=((@($_.servicePrincipalName)) -join ' ; '); 'Emplacement (OU)'=(Get-OUFromDN $_.DistinguishedName) }
} | Sort-Object 'Nom du compte'
...
$pwdRisk = $users | Where-Object { $_.Enabled -and ($_.PasswordNeverExpires -or $_.PasswordNotRequired -or (-not $_.PasswordLastSet) -or ($_.PasswordLastSet -and (Days-Since $_.PasswordLastSet) -ge $MaxPasswordAgeDays)) } | ForEach-Object {
    [PSCustomObject]@{ 'Nom du compte'=$_.SamAccountName; "MDP n'expire jamais"=(if($_.PasswordNeverExpires){'Oui'}else{'Non'}); 'MDP non requis'=(if($_.PasswordNotRequired){'Oui'}else{'Non'}); 'Âge MDP (jours)'=(Days-Since $_.PasswordLastSet); 'Emplacement (OU)'=(Get-OUFromDN $_.DistinguishedName) }
} | Sort-Object 'Âge MDP (jours)' -Descending

# ---- Groupes (synthèse) ----
$groupsReport = $groups | ForEach-Object {
    [PSCustomObject]@{ 'Nom du groupe'=$_.Name; 'Catégorie'="$($_.GroupCategory)"; 'Étendue'="$($_.GroupScope)"; 'Nb membres'=@($_.member).Count; 'Date de création'=$_.whenCreated; 'Emplacement (OU)'=(Get-OUFromDN $_.DistinguishedName) }
} | Sort-Object 'Nom du groupe'
$emptyGroups = @($groupsReport | Where-Object { $_.'Nb membres' -eq 0 }).Count
...
$trustReport = $trusts | ForEach-Object {
    $sid = if ($_.SIDFilteringQuarantined -or $_.SIDFilteringForestAware) { 'Actif' } else { 'Désactivé' }
    [PSCustomObject]@{ 'Partenaire'=$_.Target; 'Sens'="$($_.Direction)"; 'Type'="$($_.TrustType)"; 'Intra-forêt'=(if($_.IntraForest){'Oui'}else{'Non'}); 'Filtrage SID'=$sid; 'Date de création'=$_.whenCreated }
}

# ---- GPO + cpassword (optionnel) ----
$gpoReport = @(); $cpwd = New-Object System.Collections.Generic.List[object]
if (Get-Module -ListAvailable -Name GroupPolicy) {
    Import-Module GroupPolicy -ErrorAction SilentlyContinue
    try { $gpoReport = Get-GPO -All | ForEach-Object { [PSCustomObject]@{ 'Nom de la GPO'=$_.DisplayName; 'Statut'="$($_.GpoStatus)"; 'Date de création'=$_.CreationTime; 'Date de modification'=$_.ModificationTime } } | Sort-Object 'Nom de la GPO' } catch {}
    try {
        ...
        Get-ChildItem -Path $sysvol -Recurse -File -Include *.xml -ErrorAction SilentlyContinue | ForEach-Object {
            $h = Select-String -Path $_.FullName -Pattern 'cpassword="([^"]+)"' -ErrorAction SilentlyContinue
            if ($h) { $cpwd.Add([PSCustomObject]@{ 'Fichier'=$_.Name; 'Chemin'=$_.FullName; 'Risque'='CRITIQUE' }) }
        }
    } catch {}
}
...
if ($hasLegacy -or $hasNew) {
    $lp = @('Name'); if ($hasLegacy){$lp+='ms-Mcs-AdmPwdExpirationTime'}; if ($hasNew){$lp+='msLAPS-PasswordExpirationTime'}
    try { $lc = Get-ADComputer -Filter 'OperatingSystem -like "*Windows*"' -Properties $lp @baseParam
        $lapsManaged = @($lc | Where-Object { ($hasLegacy -and $_.'ms-Mcs-AdmPwdExpirationTime') -or ($hasNew -and $_.'msLAPS-PasswordExpirationTime') }).Count } catch {}
}
...
# ---- Doublons (SPN / UPN / email) ----
function Find-Dups { param($Selector,[string]$Type)
    $map = @{}; foreach ($u in $users) { foreach ($k in (& $Selector $u)) { if ([string]::IsNullOrWhiteSpace($k)){continue}; $kk="$k".ToLower().Trim(); if(-not $map.ContainsKey($kk)){$map[$kk]=New-Object System.Collections.Generic.List[object]}; $map[$kk].Add($u.SamAccountName) } }
    $rows=@(); foreach ($k in $map.Keys) { if (@($map[$k]).Count -gt 1) { $rows += [PSCustomObject]@{ 'Type'=$Type; 'Valeur'=$k; 'Occurrences'=@($map[$k]).Count; 'Comptes'=((@($map[$k])) -join ' ; ') } } }
    return $rows
}
$dups = @()
$dups += Find-Dups { param($u) @($u.servicePrincipalName) } 'SPN'
$dups += Find-Dups { param($u) ,$u.UserPrincipalName } 'UPN'
$dups += Find-Dups { param($u) $k=@(); if($u.mail){$k+=$u.mail}; foreach($p in @($u.proxyAddresses)){ if($p){$k+=($p -replace '^(?i)smtp:','')} }; $k } 'E-mail'
$dups = @($dups) | Sort-Object 'Type','Occurrences' -Descending

# ---- Échéances ----
$limit = $now.AddDays(30)
$acctExp = $users | Where-Object { $_.AccountExpirationDate -and $_.AccountExpirationDate -le $limit } | ForEach-Object { [PSCustomObject]@{ 'Type'='Compte'; 'Nom du compte'=$_.SamAccountName; 'Échéance'=$_.AccountExpirationDate } }
$pwdExp = $users | Where-Object { $_.Enabled -and -not $_.PasswordNeverExpires -and $_.'msDS-UserPasswordExpiryTimeComputed' -and $_.'msDS-UserPasswordExpiryTimeComputed' -ne 0 -and $_.'msDS-UserPasswordExpiryTimeComputed' -ne 9223372036854775807 } | ForEach-Object { $e=[datetime]::FromFileTime([int64]$_.'msDS-UserPasswordExpiryTimeComputed'); if ($e -le $limit) { [PSCustomObject]@{ 'Type'='Mot de passe'; 'Nom du compte'=$_.SamAccountName; 'Échéance'=$e } } }
$echeances = @($acctExp) + @($pwdExp) | Sort-Object 'Échéance'
...
$dcReport = foreach ($dc in $dcs) {
    ...
    [PSCustomObject]@{ 'Contrôleur'=$dc.HostName; 'Site'=$dc.Site; 'OS'=$dc.OperatingSystem; 'Ping'=(if($ping){'OK'}else{'KO'}); 'Échecs réplication'=(if($fails -lt 0){'N/A'}else{$fails}); 'État'=(if(-not $ping){'INJOIGNABLE'}elseif($fails -gt 0){'RÉPLICATION KO'}else{'OK'}) }
}
...
try {
    ...
    $EKUC = @('1.3.6.1.5.5.7.3.2','1.3.6.1.5.2.3.4','1.3.6.1.4.1.311.20.2.2','2.5.29.37.0')
...
$dash = @(
    ...
    @('Sécurité','Comptes Kerberoastables', @($kerb).Count, (& $L @($kerb).Count 1 5)),
    @('Sécurité','Mots de passe GPP (cpassword)', @($cpwd).Count, (& $L @($cpwd).Count 1 1)),
    @('Sécurité','Approbations sans filtrage SID', @($trustReport | Where-Object { $_.'Filtrage SID' -eq 'Désactivé' }).Count, (& $L @($trustReport | Where-Object { $_.'Filtrage SID' -eq 'Désactivé' }).Count 1 1)),
    ...
    @('Hygiène','Groupes vides', $emptyGroups, ''),
...
Add-Detail $compInactive  'Ordinateurs inactifs'  'PCInactifs'     @('Date de création','Dernière connexion')
...
# ---- Tableau de bord (KPI) en 1ère position ----
$wsK = Add-Worksheet -ExcelPackage $excel -WorksheetName 'Tableau de bord (KPI)' -MoveToStart
$wsK.Cells['B2'].Value = 'Rapport Active Directory - Tableau de bord'
...
foreach ($k in $dash) {
    if ($k[0] -ne $curCat) {
        $row += 1; $curCat = $k[0]
        $wsK.Cells["B$row"].Value = $curCat
        $wsK.Cells["B$row:D$row"].Merge = $true
...
# Graphique répartition OS + niveaux de risque (colonnes F/G)
